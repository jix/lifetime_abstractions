...
/// Helper traits used for the implementation.
///
/// These are not part of the public API but documented to help with understanding error messages.
pub mod helpers {
    use super::*;
    pub struct Lt<'a>(PhantomData<&'a mut ()>);

    pub trait LtSubstitute<'a> {
        type Output;
    }

    impl<'a, T, Output> LtSubstitute<'a> for T
    where
        T: FnOnce(Lt<'a>) -> Output,
    {
...
pub trait LtAbs: for<'a> LtSubstitute<'a> {}

impl<T> LtAbs for T where T: for<'a> LtSubstitute<'a> {}

/// Substitutes a concrete lifetime for the bound lifetime in an abstraction.
pub type LtApply<'a, Abs> = <Abs as LtSubstitute<'a>>::Output;

/// Creates a lifetime abstraction, binding a placeholder lifetime.
#[macro_export]
macro_rules! Lt {
    (for<$lt:lifetime> $ty:ty) => { for<$lt> fn($crate::helpers::Lt<$lt>) -> $ty };
    ($ty:ty) => { fn($crate::helpers::Lt) -> $ty };
}
